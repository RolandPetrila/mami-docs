...
**Status:** ✅ Faze 0+1+1.5+2+3+4 Complete; Faze 5-10 PENDING (sprint îmbunătățiri post-audit, derivate din `.claude-outputs/audit/`, `improve/`, `imbunatatiri/` 2026-05-06).  
**Versiune plan:** 2.1  
...
- [ ] **T5.0** — Commit fișiere modificate curent (preluare lucru anterior) [LOW] [efort: MIC]
  - **Sursa:** audit [MED-9]
...
- [ ] **T5.1** 🚨 — DOMPurify 3.1.5 → 3.4.2 [LOW] [efort: MIC]
...
- [ ] **T5.2** 🚨 — mammoth 1.8.0 → 1.12.0 [LOW] [efort: MIC]
...
- [ ] **T5.3** 🚨 — XSS în OCR (mami-image-viewer.ts:211,219,227,230) [HIGH IMPACT] [efort: MIC]
...
- [ ] **T5.4** — Tap targets 44×44px (WCAG 2.5.5) [LOW] [efort: MIC]
...
- [ ] **T5.5** — `await syncDeviceRole` (race condition) [LOW] [efort: MIC]
...
- [ ] **T5.6** — Null pointer `_wrapText` [LOW] [efort: MIC]
...
- [ ] **T5.9** — `aria-label` complet pe icon-only buttons [LOW] [efort: MIC]
...
- [ ] **T5.10** — Contrast WCAG AA — mami-search.ts:101,108 [LOW] [efort: MIC]
  - **Sursa:** audit [HIGH-7]
  - **Cum:** `color: #aaa` → `#666`, `color: #bbb` → `#777`
- [ ] **T5.11** — Memory leak listen buttons cleanup [LOW] [efort: MEDIU]
...
- [ ] **T5.12** — PDF generate try/catch [LOW] [efort: MIC]
...
- [ ] **T5.13** 🌟 — Integrare RAG în chat [MEDIUM IMPACT] [efort: MEDIU]
  - **Sursa:** audit [HIGH-8]
...
- [ ] **T5.14** — `prefers-reduced-motion` global [LOW] [efort: MIC]
  ...
  - **Cum:** `src/styles/global.css`:
    ...
    ```
- [ ] **T5.15** — Empty catch blocks (6 locații) [LOW] [efort: MIC]
...
- [ ] **T5.18** — Re-run `/audit` complet → confirmă scor ≥75 [efort: MIC]
